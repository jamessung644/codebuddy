# Codex Pets (ClaudeCo) - AI 컨텍스트 문서
...
- `sendClaude(petId, systemPrompt, message, model)` — Codex CLI를 spawn하여 실행. `--output-format stream-json` 사용. `--no-input` 미사용 (권한 승인 인터랙션 지원)
...
| `ai-approve` | renderer→main | Codex CLI stdin에 "y\n" 전송 (권한 승인) |
...
- Codex CLI는 `{ proc, waitingPermission, permissionTimer }` 구조
- 앱 종료 시 모든 프로세스 정리
...
this.aiProvider = 'Codex'; // 선택된 AI 제공자
...
3. **`#chat-panel`** — AI 채팅 패널 (우측 사이드바, 420px)
   - 모델 선택 드롭다운 (Codex/OpenAI/Gemini/Ollama, 총 10개 모델)
   - 채팅/파일 탭 전환
   - 작업 폴더 표시
   - 메시지 입력 + 전송 버튼
...
|--------|------|--------------|------------|
| Codex | 기본 모델 | `Codex` | `""` |
| Codex | Sonnet 4.6 | `Codex` | `Codex-sonnet-4-6` |
| Codex | Opus 4.6 | `Codex` | `Codex-opus-4-6` |
| Codex | Haiku 4.5 | `Codex` | `Codex-haiku-4-5-20251001` |
...
```
Codex CLI tool_use 이벤트 감지 (main.js detectPermissionNeeded)
